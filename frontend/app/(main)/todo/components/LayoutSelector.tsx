...
export default function LayoutSelector({ value, onChange }: LayoutSelectorProps) {
  ...
  const specialPresets = LAYOUT_PRESETS.filter(
    (preset) => preset.id === "good-morning" || preset.id === "everything" || preset.id === "chipping-away" || preset.id === "data-chores" || preset.id === "roulette" || preset.id === "stuff" || preset.id === "later" || preset.id === "chores"
  );
  // Sort special presets to ensure correct order: good-morning, chores, everything, roulette, stuff, later
  const specialPresetOrder = ["good-morning", "chores", "everything", "chipping-away", "data-chores", "roulette", "stuff", "later"];
