...
describe('GET /api/shows-todos', () => {
  const original = process.env.SHOW_TODOS_USER_ID;
  ...
  afterEach(() => {
    if (original === undefined) delete process.env.SHOW_TODOS_USER_ID;
    else process.env.SHOW_TODOS_USER_ID = original;
  });
  ...
  it('is enabled only for the configured user', async () => {
    process.env.SHOW_TODOS_USER_ID = '1';
  ...
  it('fails closed when SHOW_TODOS_USER_ID is unset — even for user 1', async () => {
    delete process.env.SHOW_TODOS_USER_ID;
    getUserIdFromAccessToken.mockReturnValue('1');
  ...
  it('does not treat an empty SHOW_TODOS_USER_ID as a match for a null user id', async () => {
    process.env.SHOW_TODOS_USER_ID = '';
    getUserIdFromAccessToken.mockReturnValue(null);
