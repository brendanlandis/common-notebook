...
/**
 * How long a completed task stays visible. Reads the caller's own setting, and
  ...
  try {
    ...
    try {
      recentlyCompleted = await fetchAllPages(
        token,
        `/api/tasks?filters[completed][$eq]=true&filters[completedAt][$gte]=${cutoffISO}&populate=project`
      );
    } catch (error) {
      // Don't fail the whole request if the visibility window can't be fetched.
      console.error('Failed to fetch recently completed tasks:', error);
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await strapiFetch(token, '/api/tasks?populate=project', {
      method: 'POST',
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { success: false, error: errorData.error?.message || 'Failed to create task' },
