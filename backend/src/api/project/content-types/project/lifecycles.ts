 ...
 *
 * `slug` is a plain `string` (the schema was moved off Strapi's `uid`, which is
 * globally unique — wrong for a multi-tenant app, where two users may want the
 * same slug). This hook is the single place that both (a) guarantees every
 * project gets a slug and (b) enforces uniqueness PER OWNER, auto-suffixing on
 * collision (`foo`, `foo-2`, `foo-3`, …).
 *
 ...
 * (see frontend/app/lib/moonPhaseReset.ts). If it ever runs multi-process, this
 * needs a DB-level constraint or a shared lock.
 */
