 ...
 * hook is the single place that both (a) guarantees every view gets a slug and
 * (b) enforces uniqueness PER OWNER, auto-suffixing on collision (`foo`, `foo-2`,
 * `foo-3`, …). Mirrors the world slug lifecycle.
 *
 ...
 *
 * Seeded views keep their historical slugs (`good-morning`, `everything`, …) so
 * existing `?view=<slug>` links survive; the migration passes those slugs in.
 *
 * Caveat: two *simultaneous* same-name creates by one owner could race to the
...
export default {
  ...
  async beforeUpdate(event: { params: { data?: any; where?: any } }) {
    const data = event.params.data;
    if (!data) return;
    // Only recompute when the slug or name is actually changing.
    if (data.slug == null && data.name == null) return;
    ...
    const base = slugify(data.slug || data.name || current?.name || "");
